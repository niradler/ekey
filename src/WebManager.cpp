...
void WebManager::handleKey(AsyncWebServerRequest *request)
{
    if (!_bleManager)
    {
        request->send(500, "application/json", "{\"error\":\"BLE not initialized\"}");
        return;
    }
    ...
    if (!_bleManager->isConnected()) {
        request->send(503, "application/json", "{\"error\":\"BLE not connected\"}");
        return;
    }
...
<head>
    ...
    <title>ESP32 BLE Keyboard</title>
...
<body>
    <div class="container-fluid px-3 mt-3">
        <h1 class="text-center mb-3">ESP32 BLE Keyboard</h1>
